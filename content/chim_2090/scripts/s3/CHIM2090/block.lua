...
if isPlayer then input = require 'openmw.input' end
...
local function isAttacking()
  ...
  if isPlayer then attackPlaying = attackPlaying or input.getBooleanActionValue 'Use' end

  return attackPlaying
end

local function isJumping() return s3lf.getActiveGroup(anim.BONE_GROUP.LowerBody):find 'jump' ~= nil end

local function inWeaponStance() return s3lf.getStance() == types.Actor.STANCE.Weapon end

local function normalizedFatigue()
  return util.clamp(s3lf.fatigue.current / s3lf.fatigue.base, 0.0, 1.0)
end

local function normalizedEncumbrance() return s3lf.getEncumbrance() / s3lf.getCapacity() end
...
local function playingHitstun()
  ...
  return activeLegs ~= 'knockdown' and activeLegs ~= 'knockout' and activeLegs:match '^hit' ~= nil
end
...
local function signedAngleRadians(vec1, vec2, normal)
  return math.atan2(normal * (vec1 ^ vec2), vec1 * vec2)
end
...
function BlockActor:tick(dt)
  ...
  if self.blockTimer ~= 0 then
    ...
    if not Block.isBlocking then Block.toggleBlock(true) end
...
function BlockActor:getBlockTimer()
  ...
  local block, endurance =
    math.min(100, not isCreature and s3lf.block.base or s3lf.combatSkill) / 100,
  ...
  local totalTime = self.BaseBlockTime + skillBonus + enduranceBonus - fatiguePenalty
...
function BlockActor:getMaxConsecutiveBlocks()
  if isPlayer then return end
  return self.BaseMaxBlockCount
    + util.round((s3lf.block.base * 0.02) + (s3lf.endurance.modified * 0.01))
end
...
function BlockActor:AIBlockResponse()
  ...
  if normalizedHealth < 0.1 or not self:hasBlockFatigue() then return true end
  ...
  if result >= 120 then
    if self.repeatedBlocks >= self:getMaxConsecutiveBlocks() then return end
    ...
    if result < 60 then return true end
  end
end
...
function BlockActor:shouldTurtle()
  ...
  local finalChance = baseChance + skillBonus + healthModifier - staminaPenalty
...
function Block.playBlockAnimation(enable)
  ...
  if enable then
    ...
    local equipmentSlot = (
      usingShield and s3lf.EQUIPMENT_SLOT.CarriedLeft or s3lf.EQUIPMENT_SLOT.CarriedRight
    )
...
function Block.isBlocking() return Block.state.isBlocking end
...
function Block.playBlockSound()
  ...
  if I.s3ChimCore.getWeaponHandedness() == I.s3ChimCore.Handedness.ONE and Block.usingShield() then
...
function Block.calculateMitigation()
  ...
  totalMitigation = util.clamp(totalMitigation, Block.MinimumMitigation, Block.MaximumMitigation)
  ...
  Block.debugLog(
    ([[Normalized fatigue: %.5f
            ...
            Attribute Bonus: %.5f
            Standing Still Bonus: %.2f]]):format(
      normalizedFatigue(),
      totalMitigation,
      baseMitigation,
      shieldArmor,
...
function Block.consumeFatigue(weapon, attackStrength)
  ...
  if not isPlayer then fatigueLoss = fatigueLoss * BlockActor.FatigueLossPercent end
...
local function isParrying() return Block.state.isParrying end
...
function Block.canBlock()
  ...
  if isPlayer then canBlock = canBlock and not I.UI.getMode() and not isAttacking() end
...
function Block.getSpeed()
  ...
  local weightPenalty =
    math.min(shieldWeight * Block.ShieldWeightPenalty, Block.ShieldWeightPenaltyLimit)

  return (Block.BlockSpeedBase - weightPenalty) + speedModifier * normalizedFatigue()
end
...
function Block.getBlockingItem()
  ...
  local slot = Block.usingShield() and s3lf.EQUIPMENT_SLOT.CarriedLeft
    or s3lf.EQUIPMENT_SLOT.CarriedRight
  return s3lf.getEquipment(slot)
end

local function blockBegin()
  if (isPlayer and I.UI.getMode()) or not Block.canBlock() then return end
...
local function blockIfPossible()
  ...
  if not isPlayer or not blockIsPressed() or not Block.canBlock() or I.s3ChimRoll.isRolling then
    return
  end
...
local function interruptBlock()
  ...
  if isPlayer then shouldInterrupt = shouldInterrupt or I.UI.getMode() or not blockIsPressed() end
...
local keyHandlers = {
  blockSpeed = function() return Block.getSpeed() end,
  canBlock = function() return Block.canBlock() end,
  help = function()
    return [[The block interface exposes functions related to damage mitigation, animation speed and playback,  and parry handling.
  ...
  isBlocking = function() return Block.isBlocking() end,
  mitigation = function() return Block.calculateMitigation() end,
  playBlock = function()
  ...
  usingOneHanded = function() return Block.usingOneHanded() end,
  usingShield = function() return Block.usingShield() end,
}

local function handleBlockInput(state) (state and blockBegin or blockEnd)() end
