...
function Parry.timeRemaining() return Parry.state.remainingTime end
...
function Parry.calculateParryFrames()
  ...
  local shieldFrames = math.max(
    0,
    Parry.BaseShieldFrames - math.floor(shieldWeight / Parry.PerUnitShieldWeightPenalty)
  )
  ...
  totalFrames = math.floor(totalFrames + 0.5)
...
Total parry frames: %s
  ...
  return util.clamp(totalFrames, Parry.MinParryFrames, Parry.MaxParryFrames)
end

function Parry.calculateParryFrameSeconds() return Parry.calculateParryFrames() * FRAME_DURATION end

function Parry.tick(dt)
  if core.isWorldPaused() or Parry.state.remainingTime <= 0 or usingTwoHanded() then return end
...
function Parry.getDamage(hitData)
  ...
  if not incomingDamage or incomingDamage <= 0 then return 0 end
...
local ParryInterface = {
  ...
  interface = setmetatable({}, {
    __index = function(_, key)
      if key == 'help' then
        return [[The parry interface contains functions associated with the timing and damage calculations for shield-based parrying.
...
Using these settings you may increase the parry windows for specific shields, based on character skill, or change the damage transferred back through a parry.]]
      elseif key == 'getParryTimes' then
        ...
        return ('Parry time in seconds: %.2f, in frames: %.2f, with shield: %s, whose size is %d units'):format(
...
if s3lf.actorType == 0 then
  ParryInterface.engineHandlers.onFrame = function(dt) Parry.tick(dt) end
else
  ParryInterface.engineHandlers.onUpdate = function(dt) Parry.tick(dt) end
end
