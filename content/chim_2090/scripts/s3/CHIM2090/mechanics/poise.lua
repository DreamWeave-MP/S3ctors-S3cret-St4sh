...
function Poise.calculateMaxPoise()
  ...
  for _, item in pairs(s3lf.getEquipment()) do
    ...
    if item and armorRecord then totalWeight = totalWeight + armorRecord.weight end
  end

  local equipmentPoise = math.min(totalWeight * Poise.PoisePerWeight, Poise.MaxEquipmentPoise)
...
function Poise.isBroken() return Poise.state.isBroken end
...
function Poise.recoveryTime()
  ...
  Poise.debugLog(
    ([[%s-%s: Poise Recovery Time: %.3f with %d pieces of equipment]]):format(
      s3lf.recordId,
      s3lf.id,
      recoveryTime,
      equipCount
...
function Poise.weaponPoiseDamage(attackInfo)
  ...
  local usingTwoHanded = handedness == I.s3ChimCore.Handedness.TWO
    or weaponRecord.type == weapon.type.TYPE.MarksmanCrossbow

  if usingTwoHanded then totalPoise = totalPoise * Poise.TwoHandedMult end
...
function Poise.get() return Poise.state.currentPoise end
...
local function cancelKnockdownIfDead()
  if s3lf.isDead() and s3lf.isPlaying 'knockdown' then s3lf.cancel 'knockdown' end
end
...
local textKeyHandlers = {
  knockout = function(group, _)
    ...
    Poise.debugLog(('cancelling knockdown on %s'):format(s3lf.recordId))
    s3lf.cancel(group)
  end,
  knockdown = function(_, key)
    ...
    if isPlayer then toggleAllControls(true) end
  end,
}
...
I.AnimationController.addTextKeyHandler('', function(group, key)
  ...
  if scalableSpeedGroups[group] and Poise.AlwaysScaleHitAnimSpeed then
...
if isPlayer then
  ...
  function PoiseIcon:getCurrentTexture() return self.state.textureArray[self.getFrame()] end
  ...
  function PoiseIcon:tick(dt)
    ...
    if alphaUpdated and not redrew then element:update() end
  end
  ...
  IconGroup:subscribe(async:callback(function()
    local Pos, Color, Size, Enable =
      IconGroup:get 'PoiseHUDPos',
      IconGroup:get 'PoiseHUDColor',
      IconGroup:get 'PoiseIconSize',
      IconGroup:get 'EnableIcon'
  ...
  engineHandlers = {
    onUpdate = function(dt)
      ...
      if not core.isWorldPaused() then
        if Poise.isBroken() and s3lf.getStance() ~= s3lf.STANCE.Nothing and s3lf.hasAnimation() then
      ...
      if isPlayer then PoiseIcon:tick(dt) end
    end,
  },
  eventHandlers = {
