...
function Roll:hasIFrames() return self.state.hasIFrames end
...
function Roll:updateRollAnimation(direction)
  ...
  assert(
    rollDirection,
    'Invalid Roll direction provided: '
      .. tostring(direction)
      .. ', movement direction values must be between 1-4'
  )
...
local function endRoll()
  ...
  if I.S3LockOn and not I.S3LockOn.Manager.shouldTrack() then
...
function Roll:canRoll()
  ...
  if isPlayer then canRoll = canRoll and not I.UI.getMode() end
...
local fFatigueJumpBase, fFatigueJumpMult =
  core.getGMST 'fFatigueJumpBase', core.getGMST 'fFatigueJumpMult'
...
function Roll:activate(direction)
  if not self:canRoll() or not Roll:updateRollAnimation(direction) then return false end
...
function Roll.getRollDirectionFromInput()
  if not isPlayer then
    ...
    local forwardDir = input.getRangeActionValue 'MoveForward'
      - input.getRangeActionValue 'MoveBackward'
    local horizontalDir = input.getRangeActionValue 'MoveRight'
      - input.getRangeActionValue 'MoveLeft'
...
if isPlayer then
  for index, action in ipairs {
    ...
    input.registerActionHandler(
      action,
      async:callback(function(state)
        if not Roll.DoubleTapRoll or state ~= 1 or not Roll:canRoll() then return end
  ...
  input.registerTriggerHandler(
    'CHIMRollAction',
    async:callback(function() Roll:activate(Roll.getRollDirectionFromInput()) end)
  )
end
...
local function noActions()
  if
    core.isWorldPaused()
    or (not isPlayer and not s3lf.isInActorsProcessingRange())
    or not Roll:isRolling()
  then
    return
  end
...
local interfaceKeyHandlers = {
  ...
  hasIFrames = function() return Roll:hasIFrames() end,
  isRolling = function() return Roll:isRolling() end,
  rollTypeName = function() return Roll.getRollTypeName() end,
  TYPE = RollInfo.TYPE,
}
