...
function SleepManagerA:sleepHealthRecoveryActor(sleepData)
  ...
  self.debugLog(
    ...
    s3lf.recordId,
    'Restored',
    s3lf.health.current - sleepData.oldHealth,
    'health.'
  )
end
...
function SleepManagerA:sleepHealthRecoveryPlayer(sleepData)
  ...
  s3lf.health.current =
    self.calculateNewDynamic(s3lf.health, multipliedHealth, newTotal, sleepData.oldHealth)
  self.debugLog(
    s3lf.id,
    s3lf.recordId,
    'Restored',
...
function SleepManagerA.getSleepHealthRegenBase()
  local healthMult =
    storage.globalSection('SettingsGlobal' .. modInfo.name .. 'Sleep'):get 'RestHealthMult'
...
function SleepManagerA:getSleepMagickaRegenBase(sleepHours)
  local magicMult =
    storage.globalSection('SettingsGlobal' .. modInfo.name .. 'Sleep'):get 'RestMagicMult'
...
function SleepManagerA:sleepRecoveryVanilla(sleepData)
  if self.UseVanillaFatigueFormula then
    ...
    if I.s3ChimDynamic.canRegenerateFatigue() then s3lf.fatigue.current = s3lf.fatigue.base end
    ...
    if sleepData.restOrWait then
      if I.s3ChimDynamic.canRegenerateMagicka() then
        ...
        s3lf.magicka.current =
          math.min(s3lf.magicka.base, s3lf.magicka.current + (magickaPerHour * sleepData.time))
      end
    end
    return true
  end
...
function SleepManagerA.sleepActorHandler(sleepData)
  assert(
    sleepData and sleepData.time and sleepData.restOrWait ~= nil and sleepData.oldHealth,
    's3ChimDynamic_SleepActor event requires a time value'
  )
...
function SleepManagerA.sleepPlayerHandler(sleepData)
  assert(
    sleepData
      and sleepData.time
      and sleepData.restOrWait ~= nil
      and sleepData.oldHealth
      and sleepData.sleepMultiplier,
