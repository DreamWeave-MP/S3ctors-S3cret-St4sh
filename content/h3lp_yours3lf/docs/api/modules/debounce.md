...
Register this as a player script using the [bootstrap instructions](@/h3lp_yours3lf/docs/getting-started/overview.md). Send the player's `MyModFilterChanged` event whenever the filter changes. A burst of events produces one `rebuildList` call on a later update after at least 0.25 simulation seconds without another push.
