...
local defaultMinimumSize = vector2(64, 64)
local defaultPosition = vector2(80, 80)
local defaultSize = vector2(400, 300)
...
local function window(options)
    ...
    local maximumWidth = maximumSize and maximumSize.x or huge
    local maximumHeight = maximumSize and maximumSize.y or huge
    ...
    props.size = vector2(
        clamp(initialSize.x, minimumWidth, maximumWidth),
        clamp(initialSize.y, minimumHeight, maximumHeight)
    )
    ...
    addHandler(events, 'mousePress', function(event, layout)
        ...
        local width = size.x
        local height = size.y
        local horizontalHandle = min(resizeHandle, width * 0.5)
        local verticalHandle = min(resizeHandle, height * 0.5)
        ...
        if resizable and (resizeLeft or resizeRight or resizeTop or resizeBottom) then
            resizing = true
        elseif movable and hasCaption and offset.y >= captionTop and offset.y <= captionBottom then
            moving = true
        else
            return
        end
    ...
    addHandler(events, 'mouseMove', function(event, layout)
        ...
        if clampToScreen then
            ...
            if moving then
                positionX = clamp(positionX, 0, max(0, referenceWidth - width))
                positionY = clamp(positionY, 0, max(0, referenceHeight - height))
            else
                if resizeLeft then
                    positionX =
                        clamp(positionX, 0, max(0, startPositionX + startWidth - minimumWidth))
                    width = startPositionX + startWidth - positionX
                else
                    width = min(width, max(0, referenceWidth - positionX))
                end

                if resizeTop then
                    positionY =
                        clamp(positionY, 0, max(0, startPositionY + startHeight - minimumHeight))
                    height = startPositionY + startHeight - positionY
                else
                    height = min(height, max(0, referenceHeight - positionY))
                end
        ...
        if moving then
            ...
            props.position = vector2(positionX, positionY)
        ...
        if sizeChanged then
            currentWidth = width
            currentHeight = height
            props.size = vector2(width, height)
        end
    ...
    content[#content + 1] = {
        ...
        type = ui.TYPE.Widget,
        props = {
            position = vector2(contentInset, bodyOffset),
            size = vector2(-2 * contentInset, -(contentInset + bodyOffset)),
            relativeSize = fullSize,
        },
        content = ui.content {
            column {
