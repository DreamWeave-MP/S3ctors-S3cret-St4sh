...
local function normalizedScreenPosition(value)
  if value == nil then return util.vector2(0.5, 0.5) end
  return util.vector2(clamp(value.x, 0, 1), clamp(value.y, 0, 1))
end
...
local function screenPositionTitle(argument)
  if type(argument) ~= 'table' then return nil end
...
I.Settings.registerRenderer('ScreenPosition', function(value, set, argument)
  ...
  local function openPopup()
    ...
    local panelSize =
      util.vector2(screenSize.x * panelRelativeSize.x, screenSize.y * panelRelativeSize.y)
    local pickerSize =
    ...
    local function offsetToDraft(offset)
      draft =
        util.vector2(clamp(offset.x / pickerSize.x, 0, 1), clamp(offset.y / pickerSize.y, 0, 1))
      updateMarker()
    end
    ...
    panelContent:add {
      ...
      events = {
        ...
        focusLoss = async:callback(function() dragging = false end),
      },
    }
    panelContent:add {
      props = {
