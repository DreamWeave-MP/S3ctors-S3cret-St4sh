...
for fileName in vfs.pathsWithPrefix 'playlists/' do
  if fileName:find '%.lua$' then table.insert(PlaylistFileNames, fileName) end
end
...
local function stripWhitespace(str) return str:match '^%s*(.-)%s*$' end
...
I.Settings.registerGroup {
  ...
  settings = {
    ...
    },
    {
      ...
      argument = {
        trueLabel = 'ResetButtonLabel',
        falseLabel = 'ResetButtonLabel',
        l10n = 'S3Music',
      },
