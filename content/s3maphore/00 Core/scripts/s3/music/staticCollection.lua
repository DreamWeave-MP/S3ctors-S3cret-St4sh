...
Max, Min, Next, Pairs, StrFind, StrFormat,
StrGsub, StrLower, tableRemove =
    math.ceil, coroutine.create, coroutine.resume, coroutine.status, coroutine.yield, error,
    math.max, math.min, next, pairs, string.find, string.format,
    string.gsub, string.lower, table.remove
...
local pendingAdditions, previousGridCenters, seenRecordIds, seenContentFiles, seenIds, cellObjectIds =
    {}, {}, {}, {}, {}, {}
...
local CellPresence = {
    ...
    byType = {},
    byContentFile = {},
    staticList = { recordIds = {}, contentFiles = {} },
    nearestRegion = nil,
...
local Cells, DoorDestination, GetCurrentWeather, GetExteriorCell,
IsDoor, IsTeleportDoor, Players, PresenceSection, SqLen, StaticType, StorageSet,
NPCType, CreatureType, AIFight, IsDeadFn
...
do
    ...
    for k, v in Pairs(types) do TypesToNames[v] = k end
...
local function clearCellPresence()
    ...
    clear(CellPresence.staticList.recordIds)
    clear(CellPresence.staticList.contentFiles)
...
local function saveExteriorSnapshot()
    ...
    exteriorSnapshot.staticList = CellPresence.staticList
    ...
    CellPresence.staticList = { recordIds = {}, contentFiles = {} }
end
...
local function restoreExteriorSnapshot()
    ...
    CellPresence.staticList = exteriorSnapshot.staticList
...
--- Rebuilds staticList from cellObjectIds — the authoritative record of what's
--- in the current 3×3. Called after exterior diff/sweep/gap-fill to ensure
--- staticList reflects exactly the current grid, including kept cells' statics.
local function rebuildStaticListFromCellObjectIds()
    local outRecordIds = CellPresence.staticList.recordIds
    local outContentFiles = CellPresence.staticList.contentFiles
    clear(outRecordIds)
    ...
    for _, cellData in Next, cellObjectIds do
        ...
        for j = 1, numEntries do
            if cellData.objectTypes[j] == 'Static' then
                local recordId = cellData.recordIds[j]
                if not seenRecordIds[recordId] then
                    outRecordIds[#outRecordIds + 1] = recordId
...
local function removeFromPresence(recordId, objectType, contentFile)
    local byContentFile, byRecord, byType = CellPresence.byContentFile, CellPresence.byRecord, CellPresence.byType

    local recordCount = byRecord[recordId]
    if not recordCount then
        Error(StrFormat('removeFromPresence: byRecord[%s] is nil', recordId))
    end
    ...
    if not typeCount then
        Error(StrFormat('removeFromPresence: byType[%s] is nil', objectType))
    end
    if typeCount <= 1 then
...
local function collectPresenceAndStatics(cell, cellKey)
    ...
    local targetFrames = cell.isExterior
        and TARGET_TRANSITION_FRAMES * EXTERIOR_TRANSITION_MULT
    ...
    clear(recordDeltas); clear(typeDeltas); clear(contentFileDeltas)
    local cellHasHostile

    local outRecordIds = CellPresence.staticList.recordIds
    local outContentFiles = CellPresence.staticList.contentFiles
    for i = 1, numObjects do
        local obj = objects[i]; local id, objType, recordId, contentFile =
            obj.id, obj.type, obj.recordId, obj.contentFile
        ...
        -- Exterior staticList is rebuilt from cellObjectIds by rebuildStaticListFromCellObjectIds.
        ...
        if not cellHasHostile then
            if (objType == NPCType or objType == CreatureType) and id ~= TransitioningPlayer.id then
                ...
                local threshold = objType == NPCType
                    and NPC_FIGHT_THRESHOLD or CREATURE_FIGHT_THRESHOLD
                if fightValue >= threshold and not IsDeadFn(obj) then
                    cellHasHostile = true
                end
            end
        end
        ...
        if not cellKey and not cell.region then
            nearestDoor = checkForRegion(obj, nearestDoor)
        end
    ...
    if cellKey then
        ...
        if not region and nearestDoor then
            region = DoorDestination(nearestDoor).region
        end
...
local function flushPendingAdditions(budget)
    ...
    for i = #pendingAdditions, 1, -1 do
        ...
        -- Feed staticList for scripted Static spawns
        if objType == StaticType then
            if not seenRecordIds[recordId] then
                outRecordIds[#outRecordIds + 1] = recordId
                seenRecordIds[recordId] = true
            end
...
local function cellTransitionCoroutineHandler()
    ...
    -- NOTE: staticList and seenRecordIds/seenContentFiles are NOT cleared here —
    -- they are cleared per-branch to preserve exterior statics across diffs and snapshots
    ...
    if TransitionCell.isExterior then
        ...
        if exteriorSnapshotValid and oldCenter == center then
            ...
            for offsetX = -1, 1 do
                for offsetY = -1, 1 do
                    ...
                    local isNew = (not oldCenter
                            or cellX < oldMinX or cellX > oldMaxX
                            or cellY < oldMinY or cellY > oldMaxY)
            ...
            for cellKey, cellData in Next, cellObjectIds do
                ...
                if cellX < gridX - 1 or cellX > gridX + 1
                    or cellY < gridY - 1 or cellY > gridY + 1 then
                    local numEntries = #cellData.ids
                    for j = 1, numEntries do
                        removeFromPresence(cellData.recordIds[j], cellData.objectTypes[j], cellData.contentFiles[j])
                    end
            ...
            -- Rebuild staticList from the authoritative cellObjectIds
        ...
        elseif exteriorSnapshotValid then
            -- Interior→interior: snapshot already held, just clear current counters
            clear(CellPresence.byRecord)
            clear(CellPresence.byType)
            clear(CellPresence.byContentFile)
            clear(CellPresence.staticList.recordIds)
            ...
            -- Fresh start in an interior (no exterior state to save)
            clear(CellPresence.byRecord)
            clear(CellPresence.byType)
            clear(CellPresence.byContentFile)
            clear(CellPresence.staticList.recordIds)
    ...
    engineHandlers = {
        onObjectActive = function(object)
            ...
            if
                ...
                or not object.type
                or seenIds[objectId]
                or cellTransitionCoroutine
            then
                return
            end

            local player = Players[1]; local oldCell = PreviousPlayerCells[player.id]
            if not oldCell or player.cell.id == oldCell then
                return
            end
