...
local ReadOnlyMT = {
  __newindex = function(t) error(StrFormat('Write attempt to read-only table %s', t)) end,
  __metatable = false,
}
...
local StrictReadOnlyMT = {
  ...
  __newindex = function(inTable) error(StrFormat('Write attempt to read-only table %s', inTable)) end,
  __metatable = false,
}
...
local function getPlaylistFilePaths()
  ...
  for fileName in pathsMatching 'playlists/' do
    if fileName:find '%.lua$' then TableInsert(result, fileName) end
  end

  return result
end

--- Returns all recognized files (.lua, .yaml, .yml) under Playlists/.
--- Used by the coroutine loader. Does NOT replace getPlaylistFilePaths(),
--- which is kept for the public API and returns only .lua files.
  ...
  for fileName in pathsMatching(path) do
    ...
    if includeTrack and exclusions then
      ...
      if excludedTracks and includeTrack then
        -- Whereas specific track names can match more loosely, although it is recommended to try to match the path as closely as possible
...
local function initMissingPlaylistFields(playlist, INTERRUPT)
  ...
  if playlist.randomize == nil then playlist.randomize = false end
...
local function initTracksOrder(tracks, randomize)
  ...
  for i = #tracks, 1, -1 do
    if not fileExists(tracks[i]) then TableRemove(tracks, i) end
  end
...
local function makeReadOnly(inTable, copy, strict, visited)
  ...
  if copy then
    ...
    for k in pairs(inTable) do
      if type(k) == 'table' then tableKeys[#tableKeys + 1] = k end
    end
