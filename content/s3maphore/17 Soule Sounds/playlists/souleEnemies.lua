...
local function almsiviEnemyRule()
  return Playback.state.isInCombat and Playback.rules.combatTargetExact(almsiviEnemies)
end
...
local function animalEnemyRule()
  return Playback.state.isInCombat and Playback.rules.combatTargetType(animalTypes)
end
...
local function bossFightRule()
  return Playback.state.isInCombat and Playback.rules.combatTargetExact(bossFightTargets)
end
...
local function corprusEnemyRule(playback)
  return playback.state.isInCombat and playback.rules.combatTargetExact(corprusEnemies)
end
...
local function endgameRule()
  return Playback.state.isInCombat and Playback.rules.combatTargetExact(DagothurEnemiesEnemyNames)
end

---@type ValidPlaylistCallback
local function defaultBattleRule() return Playback.state.isInCombat end
...
local function hircineenemiesEnemyRule()
  return Playback.state.isInCombat and Playback.rules.combatTargetExact(hircineNames)
end
...
local function solstheimEnemyRule()
  return Playback.state.isInCombat
    and (
      ...
      or Playback.rules.combatTargetMatch(solstheimMatches)
    )
end
