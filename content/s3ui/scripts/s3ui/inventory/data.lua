...
local function itemEquipped(actor, equippedIds, item)
	if item and item.recordId and equippedIds[item.recordId] then
		return true
	end
...
local function categoryForItem(itemType)
	if itemType == types.Weapon then
		return CATEGORY_BY_KEY.weapons
	end
	if itemType == types.Armor then
		return CATEGORY_BY_KEY.armor
	end
	if itemType == types.Clothing then
		return CATEGORY_BY_KEY.apparel
	end
	if itemType == types.Ingredient or itemType == types.Potion or itemType == types.Apparatus then
		return CATEGORY_BY_KEY.alchemy
	end
	if itemType == types.Book then
		return CATEGORY_BY_KEY.books
	end
	if itemType == types.Lockpick or itemType == types.Probe or itemType == types.Repair or itemType == types.Light then
		return CATEGORY_BY_KEY.tools
	end
...
local function subtypeName(recordType, record)
	...
	if recordType == types.Armor then
		return ARMOR_TYPE_NAMES[record.type] or EMPTY_FIELD
	end
	if recordType == types.Clothing then
		return CLOTHING_TYPE_NAMES[record.type] or EMPTY_FIELD
	end
	if recordType == types.Weapon then
		return WEAPON_TYPE_NAMES[record.type] or EMPTY_FIELD
	end
	if recordType == types.Apparatus then
