...
function M.layout(ctx, width, x, y)
	...
	return {
		...
		type = ui.TYPE.Widget,
		props = { relativePosition = v2(x, y), relativeSize = v2(width, 1), autoSize = false },
		content = content,
	}
end

function M.create(ctx, width, x, y)
	return ui.create(M.layout(ctx, width, x, y))
end

function M.update(element, ctx, width, x, y)
	if not element or not element.layout then
		return false
	end
	element.layout = M.layout(ctx, width, x, y)
	element:update()
	return true
end
