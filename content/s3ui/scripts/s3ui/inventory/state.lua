...
function M.new()
    ...
    function state:home()
        if self.scrollOffset == 0 then return false end
        self.scrollOffset = 0
        return true
    end
    ...
    function state:selectedEntryData(entries, firstIndex)
        if self.selectedSlotIndex == nil or self.selectedSlotViewMode ~= self.viewMode then return nil end
    ...
    function state:buildEntries(items, categoryOrder)
        return display.buildDisplayEntries(items, categoryOrder, self.collapsedCategories, self.sortMode, self.sortAscending)
    end
