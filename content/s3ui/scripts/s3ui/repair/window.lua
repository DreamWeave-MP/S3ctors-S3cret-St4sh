...
local function active() return rootElement and rootElement.layout and I.UI.isWindowVisible(WINDOW) end

local function selectedItem() return items[selectedIndex] end

local function clampBounds()
  local maxScroll = math.max(0, #items - builder.ROWS_VISIBLE)
...
local function resetStrike()
  ...
  meterRunning = false
  if rootElement then meter.update(rootElement, strikeState) end
end
...
local function closeMode() I.UI.removeMode(MODE) end
...
local function layoutCtx()
  ...
  return {
    ...
    armorer = armorer,
    cancel = function()
      if isAlive(callbackGeneration) then closeMode() end
    end,
    ...
    select = function(index)
      if isAlive(callbackGeneration) then M.select(index) end
    end,
    ...
    strike = strikeState,
    strikeNow = function()
      if isAlive(callbackGeneration) then M.activateStrike() end
    end,
    tool = tool,
  }
end
...
function M.rebuildElement()
  if not (rootElement and rootElement.layout) then return end
...
function rebuildLayout() return builder.make(layoutCtx()) end
...
function M.startStrike()
  if not (active() and selectedItem() and strikeState) then return false end
...
function M.activateStrike()
  if not meterRunning then return M.startStrike() end
  M.strikeSelected()
  return true
end
...
function M.navigate(delta)
  if not active() or #items == 0 then return end
  M.select(math.max(1, math.min(#items, selectedIndex + delta)))
end
...
function M.strikeSelected()
  local item = selectedItem()
  if not (item and strikeState and meterRunning) then return end
  if not tool or tool.uses <= 0 then
    lastMessage = 'The repair tool is spent.'
    meterRunning = false
    closeMode()
  ...
  local afterUses = math.max(0, beforeUses - math.max(0, wear))
  ...
  if applied > 0 then
    core.sendGlobalEvent(
      'ModifyItemCondition',
      { actor = playerObject(), item = item.item, amount = applied }
    )
  end
...
function M.update(dt)
  if not active() or not meterRunning or not strikeState or #items == 0 then return end
