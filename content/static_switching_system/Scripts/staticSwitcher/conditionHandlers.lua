...
local type = type
local StrFind, StrLower, StrFormat, StrUpper =
  string.find, string.lower, string.format, string.upper

local hasTag
if I.FlexTagG then hasTag = I.FlexTagG.objectHasTag end
...
local conditionHandlers = {
  ...
  cell_match = function(object, matchStr)
    ...
    if cellName ~= '' then cellName = StrLower(cellName) end
  ...
  creature_type = function(object, typeValue)
    ...
    if not objectRecord then return false end

    local creatureType = objectRecord.type

    if type(typeValue) == 'string' then typeValue = CREATURE_TYPE_MAP[typeValue] end

    if type(typeValue) == 'number' then return creatureType == typeValue end
  ...
  enchantment = function(object, enchantId)
    ...
    if not enchant then return false end

    if type(enchantId) == 'string' then return enchant == enchantId end

    if enchantId[1] then
      for i = 1, #enchantId do
        if enchant == enchantId[i] then return true end
      end
    end
  ...
  race = function(object, raceData)
    local objType = object.type
    if not objType or objType ~= types.NPC then return false end

    local npcRace = objType.records[object.recordId].race

    if type(raceData) == 'string' then return npcRace == raceData end

    if raceData[1] then
      for i = 1, #raceData do
        if npcRace == raceData[i] then return true end
      end
    end
  ...
  target_disposition = function(object, dispData)
    if object.type ~= types.NPC then return false end

    local disposition = types.NPC.getDisposition(object, Player)

    if type(dispData) == 'number' then return disposition >= dispData end

    if dispData.min and disposition < dispData.min then return false end
    if dispData.max and disposition > dispData.max then return false end
  ...
  has_lua_script = function(object, scriptPath)
    ...
    if type(scriptPath) == 'string' then return object:hasScript(scriptPath) end

    if scriptPath[1] then
      for i = 1, #scriptPath do
        if object:hasScript(scriptPath[i]) then return true end
      end
    end
  ...
  has_mwscript = function(object, scriptData)
    ...
    if type(scriptData) == 'string' then return mwscript == scriptData end

    if scriptData[1] then
      for i = 1, #scriptData do
        if mwscript == scriptData[i] then return true end
      end
    end
  ...
  locked = function(object, lockData)
    ...
    if not isLockedFn then return false end

    local locked = isLockedFn(object)

    local dataType = type(lockData)

    if dataType == 'boolean' then return locked == lockData end
    ...
    if dataType == 'table' then
      ...
      if lockData.min and level < lockData.min then return false end

      if lockData.max and level > lockData.max then return false end
  ...
  has_key = function(object, keyData)
    ...
    if not getKeyRecord then return false end

    local keyId = getKeyRecord(object)
    local keyRecordId = keyId and keyId.id

    if keyData == true then return keyRecordId ~= nil end
    if keyData == false then return keyRecordId == nil end

    if type(keyData) == 'string' then return keyRecordId == keyData end

    if keyData[1] then
      for i = 1, #keyData do
        if keyRecordId == keyData[i] then return true end
      end
    end
  ...
  has_trap = function(object, trapData)
    ...
    if not getTrapSpell then return false end

    local trapId = getTrapSpell(object)
    local trapRecordId = trapId and trapId.id

    if trapData == true then return trapRecordId ~= nil end
    if trapData == false then return trapRecordId == nil end

    if type(trapData) == 'string' then return trapRecordId == trapData end

    if trapData[1] then
      for i = 1, #trapData do
        if trapRecordId == trapData[i] then return true end
      end
    end
  ...
  content_file_target = function(object, targetData)
    ...
    if not objectContentFile then return false end
    ...
    for contentFile, refnums in pairs(targetData) do
      if objectContentFile == contentFile then
        for refIndex = 1, #refnums do
          if refNum == refnums[refIndex] then return true end
        end
  ...
  scale = function(object, targetScale)
    ...
    if targetScale.min and object.scale < targetScale.min then return false end
    if targetScale.max and object.scale > targetScale.max then return false end
  ...
  ['player_level'] = function(_, levelData)
    ...
    if type(levelData) == 'number' then return currentLevel >= levelData end

    if levelData.min and currentLevel < levelData.min then return false end
    if levelData.max and currentLevel > levelData.max then return false end
  ...
  ['player_attribute'] = function(_, attrData)
    ...
    for attrId, attrValue in pairs(attrData) do
      ...
      if type(attrValue) == 'number' then
        if value < attrValue then return false end
      else
        if attrValue.min and value < attrValue.min then return false end

        if attrValue.max and value > attrValue.max then return false end
      end
    end
  ...
  ['player_skill'] = function(_, skillData)
    ...
    for skillId, skillValue in pairs(skillData) do
      ...
      if type(skillValue) == 'number' then
        if value < skillValue then return false end
      else
        if skillValue.min and value < skillValue.min then return false end

        if skillValue.max and value > skillValue.max then return false end
      end
    end
  ...
  ['target_attribute'] = function(object, attrData)
    ...
    if not stats or not stats.attributes then return false end
  ...
  ['target_skill'] = function(object, skillData)
    ...
    local stats = objType.stats
    if not stats or not stats.skills then return false end
  ...
  ['player_spell'] = function(_, spellData) return BatchCache.actorSpells()[spellData] ~= nil end,
  ['global_value'] = function(_, globalData)
    ...
    for i = 1, #globalData do
      ...
      if type(varValue) == 'number' then
        if value < varValue then return false end
      else
        if varValue.min and value < varValue.min then return false end
        if varValue.max and value > varValue.max then return false end
      end
    end
  ...
  ['player_health'] = function(_, healthData)
    ...
    if type(healthData) == 'number' then return value >= healthData end

    if healthData.min and value < healthData.min then return false end
    if healthData.max and value > healthData.max then return false end
  ...
  ['player_magicka'] = function(_, magickaData)
    ...
    if type(magickaData) == 'number' then return value >= magickaData end

    if magickaData.min and value < magickaData.min then return false end
    if magickaData.max and value > magickaData.max then return false end
  ...
  ['player_fatigue'] = function(_, fatigueData)
    ...
    if type(fatigueData) == 'number' then return value >= fatigueData end

    if fatigueData.min and value < fatigueData.min then return false end
    if fatigueData.max and value > fatigueData.max then return false end
  ...
  ['time_of_day'] = function(_, hourData)
    ...
    if type(hourData) == 'number' then return currentHour >= hourData end

    local min = hourData.min
    local max = hourData.max

    if min and max and min > max then return currentHour >= min or currentHour <= max end

    if min and currentHour < min then return false end
    if max and currentHour > max then return false end
  ...
  ['player_faction'] = function(_, factionData)
    ...
    if factionData.rank then return rank >= factionData.rank end

    if factionData.min and rank < factionData.min then return false end
    if factionData.max and rank > factionData.max then return false end
  ...
  ['faction_owner_rank'] = function(object, rankData)
    ...
    if type(rankData) == 'number' then return rank >= rankData end

    if rankData.min and rank < rankData.min then return false end
    if rankData.max and rank > rankData.max then return false end
  ...
  ['target_class'] = function(object, classData)
    ...
    if type(classData) == 'string' then return StrLower(classId) == classData end

    if classData[1] then
      for i = 1, #classData do
        if StrLower(classId) == classData[i] then return true end
      end
      return false
    end
  ...
  ['player_equipped'] = function(_, equippedData)
    ...
    if type(equippedData) == 'string' then
      for _, item in pairs(equipment) do
        if item.recordId == equippedData then return true end
      end
      return false
    end
    ...
    if equippedData[1] then
      for _, item in pairs(equipment) do
        for i = 1, #equippedData do
          if item.recordId == equippedData[i] then return true end
        end
      end
      return false
    end
  ...
  ['current_weather'] = function(_, weatherData)
    ...
    if weatherData.isStorm ~= nil then
      if not current or current.isStorm ~= weatherData.isStorm then return false end
    end
    if weatherData.name then
      if
        not current or not StrFind(StrLower(current.name), StrLower(weatherData.name), 1, true)
      then
        return false
      end
    end
