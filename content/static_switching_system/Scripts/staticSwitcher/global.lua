...
processUninstall = function()
	...
	InfoLog('Uninstall complete for %s, forcing save and quit', ModuleToRemove)
...
settingsGroup:subscribe(require('openmw.async'):callback(function(_, key)
	if key == 'StaticSwitcherDisableModule' and settingsGroup:get 'StaticSwitcherDisableModule' == true then
		...
		InfoLog('Uninstall triggered via settings for: %s', moduleToUninstall)
...
return {
	...
	engineHandlers = {
		...
		onObjectActive = function(object)
			...
			DebugLog('Object active: %s (%s) [stack=%d]', object.id, object.recordId or '?', #ActiveObjectStack)
		...
		onLoad = function(data)
			...
			if settingsGroup:get 'StaticSwitcherDisableModule' then
				InfoLog 'Clearing stale disable-module flag after load'
