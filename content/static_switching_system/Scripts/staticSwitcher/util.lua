...
---@param object any
local function deepLog(object) print(LogString(aux_util.deepToString(object, 5))) end
...
local function getMeshPath(path)
  ...
  if not StrMatch(path, '^meshes/') then path = 'meshes/' .. path end
...
---@param prefix string?
function Log(message, prefix) print(LogString(message, prefix)) end
...
local function Record(object) return object.type.records[object.recordId] end
...
local function capitalize(inputString)
  ...
  if stringLength <= 1 then return StrUpper(inputString) end

  return StrFormat(
    TITLE_CAP_FORMAT_STR,
    StrUpper(StrSub(inputString, 1, 1)),
    StrSub(inputString, 2, stringLength)
  )
end
