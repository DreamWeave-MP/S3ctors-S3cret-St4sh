...
| `CheckLOS` | boolean | `false` | Continues checking line of sight after acquisition. Initial acquisition always checks visibility. |
...
| `TargetMinSize` | integer | `32` | Marker size at or below `TargetMinDistance`. |
| `TargetMinDistance` | integer | `256` | Distance at which the marker reaches minimum size. |
| `TargetMaxSize` | integer | `128` | Marker size at `TargetMaxDistance`. |
| `TargetMaxDistance` | integer | `3564` | Maximum lock-on distance; the marker reaches its maximum configured size here. |
