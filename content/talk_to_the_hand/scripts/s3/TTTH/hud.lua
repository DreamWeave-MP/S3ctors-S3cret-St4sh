...
function colorUtil.overlay(base, blend)
  ...
  local blend_rgb = blend:asRgb()
  local mix = util.vector3(
    base_rgb.x < 0.5 and (2 * base_rgb.x * blend_rgb.x)
      or (1 - 2 * (1 - base_rgb.x) * (1 - blend_rgb.x)),
    base_rgb.y < 0.5 and (2 * base_rgb.y * blend_rgb.y)
      or (1 - 2 * (1 - base_rgb.y) * (1 - blend_rgb.y)),
    base_rgb.z < 0.5 and (2 * base_rgb.z * blend_rgb.z)
      or (1 - 2 * (1 - base_rgb.z) * (1 - blend_rgb.z))
  )
  return util.color.rgb(mix.x, mix.y, mix.z)
end
...
function colorUtil.softlight(base, blend)
  ...
  local mix = util.vector3(
    blend_rgb.x < 0.5
        and (2 * base_rgb.x * blend_rgb.x + base_rgb.x * base_rgb.x * (1 - 2 * blend_rgb.x))
      or (2 * base_rgb.x * (1 - blend_rgb.x) + math.sqrt(base_rgb.x) * (2 * blend_rgb.x - 1)),
    blend_rgb.y < 0.5
        and (2 * base_rgb.y * blend_rgb.y + base_rgb.y * base_rgb.y * (1 - 2 * blend_rgb.y))
      or (2 * base_rgb.y * (1 - blend_rgb.y) + math.sqrt(base_rgb.y) * (2 * blend_rgb.y - 1)),
    blend_rgb.z < 0.5
        and (2 * base_rgb.z * blend_rgb.z + base_rgb.z * base_rgb.z * (1 - 2 * blend_rgb.z))
      or (2 * base_rgb.z * (1 - blend_rgb.z) + math.sqrt(base_rgb.z) * (2 * blend_rgb.z - 1))
  )
  return util.color.rgb(mix.x, mix.y, mix.z)
end
...
---@return openmw.util.Color
function OrbGen.vectorColor(vector) return util.color.rgb(vector.x, vector.y, vector.z) end
...
function OrbGen:getColor(inColor)
  ...
  for _, layer in ipairs(order) do
    ...
    if layer.hue then color = colorUtil.hueShift(color, layer.hue) end
...
function OrbGen:subElement(elementInfo)
  ...
  if elementInfo.twoSided and elementInfo.rightSide then xPos = -1 end
...
function OrbGen:container(constructor)
  ...
  return {
    ...
    content = ui.content {
      ...
      OrbGen:lid(
        colorUtil.hueShift(colors.noise, -25),
        constructor.twoSided,
...
function OrbGen:update(orb)
  ...
  for _, layout in ipairs(orbBase) do
    ...
    local height = util.clamp(s3lf[stat].current / s3lf[stat].base, 0.0, 1.)

    local newHeightInt, currentHeightInt =
      util.round(height * 100), util.round(layout.props.relativeSize.y * 100)
    ...
    for _, elementName in ipairs(OrbGen.updateOrder) do
      ...
      if updateHeight then childElement.props.relativeSize = childSize end
      ...
      if childElement.userData and childElement.userData.atlas then
        ...
        if data.currentFrame == 0 then
          data.currentFrame = 100
        elseif data.currentFrame == 101 then
          data.currentFrame = 1
        end
...
local HudCore, Compass, ThumbAtlas, MiddleAtlas, PinkyAtlas, CompassAtlas, CastableIndicator, WeaponIndicator, EffectBar, MagFatOrb, HealthOrb, TotalDelay, CurrentDelay
...
function H4ND.updateUIFramerate() TotalDelay = 1 / H4ND.UIFramerate end
...
function H4ND.getStatAtlas(statName)
  if not StatMap then
    assert(
      MiddleAtlas and PinkyAtlas and ThumbAtlas,
...
function H4ND:getCompassPath() return ('textures/s3/TTTH/compass/%s.dds'):format(self.CompassStyle) end
...
function EffectBarManager:getActiveEffectIds()
  ...
  for _, spell in pairs(s3lf.activeSpells()) do
    for _, effect in ipairs(spell.effects) do
      ...
      if
        not storedEffect
        or (storedEffect.durationLeft or math.huge) < (effect.durationLeft or math.huge)
      then
...
function EffectBarManager:constructEffectImages()
  ...
  local currentRowNum, totalRows = 1, math.max(1, math.floor(#effects / H4ND.EffectsPerRow + 0.5))
  ...
  for _, magicEffect in ipairs(effects) do
    ...
    local alpha = util.clamp((magicEffect.durationLeft or 1) / (magicEffect.duration or 1), 0.0, 1.)
...
local function getItemIcon(item) return item.type.records[item.recordId].icon end
...
local function getSelectedWeaponIcon()
  ...
  if not weapon then return H2HIcon end
...
function H4ND.updateTime() H4ND.state.lastUpdateTime = core.getRealTime() end
...
local function updateWeaponDurability()
  ...
  local currentWidth = math.floor(
    WeaponIndicator.layout.content.WeaponIndicator.content.DurabilityBarContainer.content.DurabilityBar.props.relativeSize.x
  ) * 100
...
local function updateCastableBar()
  ...
  local currentWidth = math.floor(
    CastableIndicator.layout.content.CastableIndicator.content.CastChanceContainer.content.CastChanceBar.props.relativeSize.x
  ) * 100
...
local function updateDurabilityBarSize()
  WeaponIndicator.layout.content.WeaponIndicator.content.DurabilityBarContainer.content.DurabilityBar.props.relativeSize =
...
local function handleFade()
  ...
  if not hudVisible and CompassProps.alpha > 0. then
    CompassProps.alpha = math.max(CompassProps.alpha - H4ND.FadeStep, 0.0)
    Compass:update()
  ...
  if ((hudIsStale and H4ND.UseFade) or not hudVisible) and HudProps.alpha > 0.0 then
    step = math.max(HudProps.alpha - H4ND.FadeStep, 0.0)
  elseif (fadeIn or hudVisible) and HudProps.alpha ~= 1. then
    step = math.min(1., HudProps.alpha + H4ND.FadeStep)

    if step == 1. then H4ND.updateTime() end
  else
    return
  end
  ...
  return {
    ...
    mouseMove = async:callback(function(mouseEvent, layout)
      ...
      if layout.userData.scaleDrag then
        ...
        local goLeft = layout.props.anchor.x <= 0.5
        local goDown = layout.props.anchor.y <= 0.5
      ...
      local newX, newY =
        util.clamp(scalar.x + relativeDelta.x, 0.0, 1.),
        util.clamp(scalar.y + relativeDelta.y, 0.0, 1.)
...
CompassAtlas, ThumbAtlas, MiddleAtlas, PinkyAtlas =
  respawnCompassAtlas(), require 'scripts.s3.TTTH.atlasses'(Constants, H4ND.getColorForElement)
...
H4ND.state.resizeHandlers = {
  ...
  [EffectBar] = function(resizeInfo) H4ND.EffectBarSize = resizeInfo.resolvedDelta end,
  [HudCore] = function(resizeInfo) H4ND.H4NDWidth = resizeInfo.resolvedDelta.x end,
  [WeaponIndicator] = function(resizeInfo) H4ND.WeaponIndicatorSize = resizeInfo.resolvedDelta.x end,
  [MagFatOrb] = function(resizeInfo) H4ND.OrbTwoSidedSize = resizeInfo.resolvedDelta.x end,
  [HealthOrb] = function(resizeInfo) H4ND.OrbOneSidedSize = resizeInfo.resolvedDelta.x end,
}

if H4ND.UIDebug then s3lf:sendEvent('SetUiMode', { mode = 'Interface', windows = {} }) end
...
return {
  ...
  eventHandlers = {
    ...
    H4NDUpdateCastableBar = function()
      CastableIndicator.layout.content.CastableIndicator.content.CastChanceContainer.content.CastChanceBar.props.relativeSize =
    ...
    H4NDUpdateWeapon = function()
      local weaponIndicator =
        WeaponIndicator.layout.content.WeaponIndicator.content.WeaponIconBox.content
  ...
  engineHandlers = {
    onFrame = function()
      ...
      if not H4ND.UIDebug then handleFade() end
