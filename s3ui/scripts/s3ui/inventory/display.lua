...
local function sortItems(items, sortMode, sortAscending)
	table.sort(items, function(left, right)
		...
		if leftValue ~= rightValue then
			if sortAscending[sortMode] then
	...
	for _, category in ipairs(categoryOrder) do
		if category.key ~= 'all' then
			...
			if #categoryItems > 0 then
				entries[#entries + 1] =
					categoryEntry(category, #categoryItems, collapsedCategories[category.key] == true)
...
function M.scrollOffsetForSelection(targetIndex, direction, scrollOffset, viewMode, metrics)
	...
	if targetIndex > scrollOffset and targetIndex <= scrollOffset + capacity then
		return scrollOffset
	end
	...
	if direction > 0 then
		return rowStart - (metrics.gridRows - 1) * metrics.gridColumns
	end
	return rowStart
end
