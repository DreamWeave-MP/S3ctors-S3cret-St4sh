...
local function processDevReloadReopen()
  ...
  if not I.UI.isWindowVisible(WINDOW) then I.UI.setMode(MODE, { windows = { WINDOW } }) end
end

local function emptyWindowOverride() end

local function closeJournalMode()
  if I.UI.getMode() == I.UI.MODE.Journal then I.UI.removeMode(I.UI.MODE.Journal) end
end
...
input.registerTriggerHandler(
  'Journal',
  async:callback(function() async:newUnsavableSimulationTimer(0, closeJournalMode) end)
)
input.registerTriggerHandler(
  'Activate',
  async:callback(function()
    if repairWindow.active() then repairWindow.activateStrike() end
  end)
)
...
return {
  engineHandlers = {
    ...
    onFrame = function(dt) repairWindow.update(dt) end,
    onKeyPress = function(key)
      ...
      elseif travelWindow.handleKeyPress(key) then
        return
      elseif
        key.code == input.KEY.Enter
        or key.code == input.KEY.NP_Enter
        or key.code == input.KEY.Space
      then
    ...
    onMouseWheel = function(vertical, _horizontal)
      if countModal.isOpen() then return end
