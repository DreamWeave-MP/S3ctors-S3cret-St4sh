...
local function button(ctx, name, label, callback)
  ...
  return {
    ...
    events = {
      focusGain = ctx.async:callback(function() ctx.setHovered(name) end),
      focusLoss = ctx.async:callback(function() ctx.clearHovered(name) end),
...
local function itemRow(ctx, repairItem, index)
  ...
  local percent = math.floor(repairItem.conditionPercent * 100 + 0.5)
  local content = ui.content {
    {
  ...
  return {
    ...
    events = {
      focusGain = ctx.async:callback(function() ctx.select(index) end),
      mouseClick = ctx.async:callback(function() ctx.select(index) end),
    },
    content = content,
  }
end
...
local function itemRows(ctx)
  ...
  local last = math.min(#ctx.items, ctx.scrollOffset + ROWS_VISIBLE)
...
local function selectedPanel(ctx)
  ...
  return {
    ...
    content = ui.content {
      text(
        'Selected Damage: Bent Edge',
        { textSize = 18, relativeSize = v2(1, 0), size = v2(0, 30) }
      ),
