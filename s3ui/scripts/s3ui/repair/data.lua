...
local function currentActor() return self.object or self end
...
local function maxCondition(record)
  if record and type(record.health) == 'number' and record.health > 0 then return record.health end
  return nil
end
...
function M.armorerSkill()
  ...
  if ok and stat and type(stat.modified) == 'number' then return stat.modified end
  return 35
end
...
function M.collectRepairableItems()
  ...
  for _, item in ipairs(inventory:getAll()) do
    ...
    if kind and item.recordId then
      ...
      local condition = itemData and type(itemData.condition) == 'number' and itemData.condition
        or max
      if max and condition and condition < max then
        rows[#rows + 1] = {
          ...
          condition = condition,
          maxCondition = max,
          conditionPercent = math.clamp(condition / max, 0, 1),
...
function M.applyConditionGain(repairItem, amount)
  ...
  local after = math.min(repairItem.maxCondition, before + math.max(0, amount))
  repairItem.condition = after
  repairItem.conditionPercent = math.clamp(after / repairItem.maxCondition, 0, 1)
...
function M.consumeToolUses(toolInfo, amount)
  if not toolInfo then return 0 end
  ...
  local after = math.max(0, before - math.max(0, amount))
  toolInfo.uses = after
  return before - after
end
