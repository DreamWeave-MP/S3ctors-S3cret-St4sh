...
function M.new(armorer, toolQuality)
  local skill = math.clamp(armorer, 0, 100) / 100
  local quality = math.clamp(toolQuality, 0.5, 2)
  return {
    marker = 0,
    direction = 1,
...
function M.rating(state)
  local distance = math.abs(state.marker - CENTER)
...
function M.conditionGain(repairItem, armorer, toolQuality, performance)
  local skillFactor = 0.55 + math.clamp(armorer, 0, 100) / 100 * 0.75
  local toolFactor = math.clamp(toolQuality, 0.5, 2)
  local base = math.max(4, repairItem.maxCondition * 0.09)
  return math.max(1, math.floor(base * skillFactor * toolFactor * performance + 0.5))
end
...
function M.toolWear(toolQuality, wearMultiplier)
  local quality = math.clamp(toolQuality, 0.5, 2)
  return math.max(1, math.floor((2.4 / quality) * wearMultiplier + 0.5))
end
